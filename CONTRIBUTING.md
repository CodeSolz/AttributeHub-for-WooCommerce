# AttributeHub for WooCommerce Development Guidelines
...
To maintain a clean and understandable project history, we follow strict rules for writing git commit messages. These messages not only enhance readability but are also used to automatically generate the **AttributeHub for WooCommerce changelog**.
...
Example:
```
fix(mapping): correct query expansion for multi-value attribute groups
```
